...
const catalogLbl: string = '[class*="NavbarCatalog_label"]';
...
const tendersSearchInput: string = '[class*="Navbar_search"] *[data-testid="search"]';
...
const searchHistory: string = '//*[text()="Історія пошуку"]/following-sibling::div[1]';
...
export class NavBar extends Page {
    ...
    async getCatalogLabel() {
        return await super.getElement(catalogLbl);
    }
    ...
    async getTendersJobRequestsSearchInput() {
        return await super.getElement(tendersSearchInput);
    }
    ...
    async getSearchHistoryPopup() {
        return await super.getElement(searchHistory);
    }
    ...
    async clickCatalog() {
        await super.clickElement(catalogLbl);
    }
