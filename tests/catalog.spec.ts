...
import catalogMenuItems from '../fixtures/menuItems.json';
import catalogMenuServices from '../fixtures/menuServices.json';
import catalogMenuVehicles from '../fixtures/menuVehicles.json';
...
test.describe('Catalog menu check', () => {
    ...
    test('Verify catalog', async () => {
        ...
        for (let i = 0; i < catalogMenuVehicles.subMenu.length; i++) {
            ...
            await expect(await productsPage.getSelectedFilters()).toHaveText(catalogMenuVehicles.subMenu[i].name);
        ...
        for (let i = 0; i < catalogMenuServices.subMenu.length; i++) {
            ...
            await expect(await productsPage.getSelectedFilters()).toHaveText(catalogMenuServices.subMenu[i].searchLabel, { ignoreCase: true });
